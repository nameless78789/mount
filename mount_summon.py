...
from constants import (
    ...
    RARITY_SCORE, get_gradient_color, get_substat_value
)
...
def load_mount_data():
    try:
        with open('MountSummonConfig.json', 'r', encoding='utf-8') as f:
            summon_config = json.load(f)
        with open('MountLibrary.json', 'r', encoding='utf-8') as f:
            mount_library = json.load(f)
        return summon_config, mount_library
    except FileNotFoundError:
        print("에러: MountSummonConfig.json 또는 MountLibrary.json을 찾을 수 없습니다.")
        return None, None

def main():
    config_data = load_mount_data()
    if config_data == (None, None):
        return
    summon_config, mount_library = config_data
    ...
    for key, data in sorted(mount_library.items(), key=lambda x: x[1]["MountId"]["Id"]):
        r = data["MountId"]["Rarity"]
        mounts_by_rarity[r].append(key)

    while True:
        print("\n" + "="*45)
        print("🐎 Forge Master Mount(탈것) 시뮬레이터")
        print("="*45)
        ...
        while True:
            ...
            if user_cmd == 'quit':
                ...
                print("\n[명령어 목록]")
                print("- 1, 15, 50 : 입력한 횟수만큼 탈것 소환을 진행합니다.")
            ...
            while current_pull_idx < total_target_pulls:
                ...
                for _ in range(num_substats):
                    stat_idx = pcg_summon.next_int(len(available_stats))
                ...
                clean_display = f"{colored_r} | {s1_padded} | {s2_padded} | {colored_p}{bonus_str}"
